...
Drupal.behaviors.rubik = function(context) {
  ...
  $('div.column-main').find('div.buttons').parents('div.form').not('.rubik-processed').each(function() {
    var form = $(this);
    var offset = $('div.column-side', form).find('div.buttons').height() + $('div.column-side', form).find('div.buttons').offset().top;
    $(window).scroll(function () {
      if ($(this).scrollTop() > offset) {
        $('div.column-main', form).find('div.buttons').show();
      }
      else {
        $('div.column-main', form).find('div.buttons').hide();
      }
    });
    form.addClass('rubik-processed');
  });

  $('a.toggler', context).not('.rubik-processed').each(function() {
    var $this = $(this);
